...
/* ---------------------------------------------------------------
   Persistence helpers
   - "Mine lister" (personal) live in the browser's localStorage.
   - "Delte lister" (shared) live in Supabase, one row per code.
--------------------------------------------------------------- */
...
async function fetchSharedBoard(code) {
  const { data, error } = await supabase.from("shared_lists").select("*").eq("code", code).maybeSingle();
  if (error) { console.error(error); return null; }
  return data;
}
async function upsertSharedBoard(code, name, items, favorites) {
  const { error } = await supabase.from("shared_lists").upsert({
    code, name, items, favorites, updated_at: new Date().toISOString(),
  });
...
export default function App() {
  ...
  // board registries (always local to this device)
  ...
  // ---- load personal settings + reopen last board ----
  useEffect(() => {
    (async () => {
      const mb = loadLocal("shopflow:myBoards", []);
      const jb = loadLocal("shopflow:joinedBoards", []);
      const st = loadLocal("shopflow:store", "netto");
      const nm = loadLocal("shopflow:name", "");
      const ord = loadLocal("shopflow:orders", DEFAULT_ORDERS);
      const last = loadLocal("shopflow:lastBoard", null);
  ...
  // ---- realtime: live-sync while a shared board is open ----
  useEffect(() => {
    if (!activeBoard || activeBoard.kind !== "shared") return;
    const channel = supabase
      .channel(`shared_lists_${activeBoard.code}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "shared_lists", filter: `code=eq.${activeBoard.code}` },
        (payload) => {
          if (payload.new) {
            setItems(payload.new.items || []);
            setFavorites(payload.new.favorites || []);
          }
        }
      )
      .subscribe();
  ...
  function persistMyBoards(next) { setMyBoards(next); saveLocal("shopflow:myBoards", next); }
  function persistJoinedBoards(next) { setJoinedBoards(next); saveLocal("shopflow:joinedBoards", next); }
  function persistOrders(next) { setOrders(next); saveLocal("shopflow:orders", next); }
  function persistStore(next) { setStore(next); saveLocal("shopflow:store", next); }
  function persistName(next) { setName(next); saveLocal("shopflow:name", next); }

  async function openBoard(ref) {
    setActiveBoard(ref);
    setScreen("board");
    setTab("liste");
    saveLocal("shopflow:lastBoard", ref);
    if (ref.kind === "personal") {
      const content = loadLocal(`shopflow:board:${ref.id}`, { items: [], favorites: [] });
      setItems(content.items || []);
      setFavorites(content.favorites || []);
    } else {
      const data = await fetchSharedBoard(ref.code);
  ...
  function goHome() {
    setScreen("home");
    setActiveBoard(null);
    saveLocal("shopflow:lastBoard", null);
  }

  function saveBoardContent(nextItems, nextFavorites) {
    if (!activeBoard) return;
    if (activeBoard.kind === "personal") {
      saveLocal(`shopflow:board:${activeBoard.id}`, { items: nextItems, favorites: nextFavorites });
    } else {
      upsertSharedBoard(activeBoard.code, activeBoard.name, nextItems, nextFavorites);
    }
  }
  ...
  async function startJoin(codeRaw) {
    ...
    if (!codeNorm) return;
    const data = await fetchSharedBoard(codeNorm);
    if (data && data.name) {
      finalizeJoin(codeNorm, data.name);
    } else {
  ...
  async function confirmNewSharedName(rawName) {
    ...
    await upsertSharedBoard(pendingCode, trimmed, [], []);
    ...
    return (
      <div style={styles.page}>
        ...
        <div style={styles.noteBox}>
          ...
          <span>Mine lister ligger kun på denne enhed. Delte lister følger koden og synkroniserer live via Supabase — alle der indtaster samme kode ser og redigerer den samme liste.</span>
        </div>
      </div>
    );
  }
          ...
          {editingRoute && (
            <div style={{ marginBottom: 24 }}>
              <div style={styles.sectionLabel}>
                {STORES.find(s => s.key === editingRoute).label} — din rute
              </div>
              <div style={styles.itemsCard}>
...
const styles = {
  page: {
    ...
    padding: "20px 16px 84px", position: "relative", minHeight: "100vh", boxSizing: "border-box",
  },
  ...
  addInput: {
    ...
    padding: "0 14px", fontFamily: BODY, fontSize: 15, color: INK, outline: "none", boxSizing: "border-box",
  },
  addBtn: {
  ...
  nav: {
    position: "fixed", left: "50%", transform: "translateX(-50%)", bottom: 12, width: "calc(100% - 24px)", maxWidth: 436,
    height: 60, background: CARD, border: `0.5px solid ${LINE}`, borderRadius: 16, display: "flex",
    boxShadow: "0 2px 12px rgba(20,20,10,0.08)",
  },
  navBtn: (active) => ({
